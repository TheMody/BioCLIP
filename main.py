...
def make_collate_fn(class_names):
...
def main():
    ...
    class_names = trainset.classes  # fine‑label names (CIFAR‑100 has 100)

   # print(class_names)
    testset = CIFAR100(root="data", train=False, download=True, transform=transform)

    train_loader = DataLoader(trainset, batch_size=batch_size*gradient_accumulation_steps, shuffle=True,
                              collate_fn=make_collate_fn(class_names), num_workers=4, pin_memory=True)
    test_loader = DataLoader(testset, batch_size=batch_size, shuffle=False,
                             collate_fn=make_collate_fn(class_names), num_workers=4, pin_memory=True)
    scheduler = CosineWarmupScheduler(optimizer, warmup=100, max_iters=epochs*len(train_loader), min_factor=0.1)
    ...
    for epoch in range(epochs):
        model.train()
        total_loss = 0.0
        with tqdm(total=len(train_loader), desc="Training,epoch " + str(epoch) ) as pbar:
            step = 0
            for images, texts in train_loader:
                ...
                for i in range(gradient_accumulation_steps):
                    ...
                    logits = model(sub_images, **token_batch)
